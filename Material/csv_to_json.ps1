# CSV to JS 변환 스크립트 (Material용)
$scriptPath = Split-Path -Parent $MyInvocation.MyCommand.Path
Set-Location $scriptPath

$csvPath = Join-Path $scriptPath "Material.csv"
$jsPath = Join-Path $scriptPath "Material.js"

if (-not (Test-Path $csvPath)) {
    Write-Host "Material.csv 파일이 없습니다." -ForegroundColor Yellow
    exit
}

Write-Host "변환 중: Material.csv" -ForegroundColor Cyan
...
try {
    ...
    Write-Host "  -> Material.js 완료 (변수: $varName)" -ForegroundColor Green
}
catch {
